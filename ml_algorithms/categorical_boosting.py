# # ============================================================
# # Import libraries
# # ============================================================
# import pandas as pd
# from catboost import CatBoostClassifier,CatBoostRegressor
# from sklearn.model_selection import train_test_split
# from sklearn.metrics import accuracy_score, classification_report, mean_squared_error, r2_score

# # ============================================================
# # Load  dataset
# # ============================================================
# df=pd.read_csv("C:/Users/devik/OneDrive/Desktop/Coderzon/music_with_listen_percentage.csv")
# print(df.columns)

# # ============================================================
# # Define X and target variables
# # ============================================================
# X = df.drop(['mood_auto', 'listen_percentage'], axis=1)

# y_class = df['mood_auto']              # Classification target
# y_reg = df['listen_percentage']        # Regression target

# # ============================================================
# # 1. CatBoost CLASSIFIER (Predict mood)
# # ============================================================
# X_train, X_test, y_train, y_test = train_test_split(
#     X, y_class, test_size=0.2, random_state=42
# )

# clf = CatBoostClassifier(
#     iterations=500,
#     learning_rate=0.05,
#     depth=8,
#     loss_function='MultiClass',
#     verbose=False
# )

# clf.fit(X_train, y_train)

# y_pred = clf.predict(X_test)

# print("\n==== CatBoost CLASSIFIER ====")
# print("Accuracy:", accuracy_score(y_test, y_pred))
# print("\nClassification Report:\n", classification_report(y_test, y_pred))

# # ============================================================
# # 2. CatBoost REGRESSOR (Predict listen_percentage)
# # ============================================================
# X_train_r, X_test_r, y_train_r, y_test_r = train_test_split(
#     X, y_reg, test_size=0.2, random_state=42
# )

# reg = CatBoostRegressor(
#     iterations=500,
#     learning_rate=0.05,
#     depth=8,
#     loss_function='RMSE',
#     verbose=False
# )

# reg.fit(X_train_r, y_train_r)

# y_pred_r = reg.predict(X_test_r)

# print("\n==== CatBoost REGRESSOR ====")
# print("RMSE:", mean_squared_error(y_test_r, y_pred_r))
# print("R2 Score:", r2_score(y_test_r, y_pred_r))

import pandas as pd
import numpy as np
from sklearn.preprocessing import LabelEncoder
from sklearn.model_selection import train_test_split
from sklearn.metrics import mean_squared_error, r2_score, accuracy_score, classification_report
from catboost import CatBoostRegressor, CatBoostClassifier

# =========================
# 1. LOAD DATA
# =========================
df = pd.read_csv("C:/Users/devik/Downloads/all_features_traffic_dataset.csv")

# =========================
# 2. DATETIME FEATURES
# =========================
if "Timestamp" in df.columns:
    df["Timestamp"] = pd.to_datetime(df["Timestamp"], errors="coerce")
    df["Hour"] = df["Timestamp"].dt.hour
    df["Day"] = df["Timestamp"].dt.day
    df["Month"] = df["Timestamp"].dt.month
    df["Year"] = df["Timestamp"].dt.year
    df["Weekday"] = df["Timestamp"].dt.weekday

# =========================
# 3. DROP CONSTANT COLUMNS
# =========================
df = df.loc[:, df.nunique() > 1]

# =========================
# 4. REGRESSION: Travel_Time
# =========================
regression_target = "Travel_Time"
X_reg = df.drop(columns=[regression_target], errors="ignore")
y_reg = df[regression_target]

# Detect categorical columns
cat_cols_reg = X_reg.select_dtypes(include=['object']).columns.tolist()
num_cols_reg = X_reg.select_dtypes(include=['float', 'int']).columns.tolist()

# Fill missing values
X_reg[num_cols_reg] = X_reg[num_cols_reg].fillna(X_reg[num_cols_reg].mean())
X_reg[cat_cols_reg] = X_reg[cat_cols_reg].fillna("missing")

# Train-test split
X_reg_train, X_reg_test, y_reg_train, y_reg_test = train_test_split(
    X_reg, y_reg, test_size=0.2, random_state=42
)

# Initialize and train CatBoost Regressor
catboost_reg = CatBoostRegressor(
    iterations=400,
    depth=8,
    learning_rate=0.05,
    cat_features=cat_cols_reg,
    verbose=0
)
catboost_reg.fit(X_reg_train, y_reg_train)

# Predict and evaluate
reg_preds = catboost_reg.predict(X_reg_test)
print("\n=== CatBoost Regression (Travel_Time) ===")
print("RMSE:", np.sqrt(mean_squared_error(y_reg_test, reg_preds)))
print("R2 Score:", r2_score(y_reg_test, reg_preds))


# =========================
# 5. CLASSIFICATION: Congestion_Level
# =========================
classification_target = "Congestion_Level"
X_clf = df.drop(columns=[classification_target], errors="ignore")
y_clf = df[classification_target]

# Encode target
le_clf = LabelEncoder()
y_clf = le_clf.fit_transform(y_clf)

# Detect categorical columns
cat_cols_clf = X_clf.select_dtypes(include=['object']).columns.tolist()
num_cols_clf = X_clf.select_dtypes(include=['float', 'int']).columns.tolist()

# Fill missing values
X_clf[num_cols_clf] = X_clf[num_cols_clf].fillna(X_clf[num_cols_clf].mean())
X_clf[cat_cols_clf] = X_clf[cat_cols_clf].fillna("missing")

# Train-test split
X_clf_train, X_clf_test, y_clf_train, y_clf_test = train_test_split(
    X_clf, y_clf, test_size=0.2, random_state=42
)

# Initialize and train CatBoost Classifier
catboost_clf = CatBoostClassifier(
    iterations=400,
    depth=8,
    learning_rate=0.05,
    cat_features=cat_cols_clf,
    verbose=0
)
catboost_clf.fit(X_clf_train, y_clf_train)

# Predict and evaluate
clf_preds = catboost_clf.predict(X_clf_test)
print("\n=== CatBoost Classification (Congestion_Level) ===")
print("Accuracy:", accuracy_score(y_clf_test, clf_preds))
print("\nClassification Report:\n", classification_report(y_clf_test, clf_preds))
